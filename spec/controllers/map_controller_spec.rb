...
RSpec.describe MapController, type: :controller do
  describe '#index' do
    before do
      @states = [
        instance_double(State, name: 'State', std_fips_code: '01'),
        instance_double(State, name: 'State', std_fips_code: '02')
      ]
    ...
    it 'assigns all states to @states and indexes them by FIPS code' do
      ...
      expect(assigns(:states_by_fips_code)).to eq(@states.index_by { |state| state.std_fips_code.rjust(2, '0') })
    end
  end
  ...
  describe '#state' do
    before do
      ...
      @counties = [instance_double(County, std_fips_code: '001', state: @state)]

      allow(State).to receive(:find_by).and_return(@state)
      allow(@state).to receive(:counties).and_return(@counties)
      allow(@counties).to receive(:index_by).and_return({ '001' => @counties[0] })
    end
    ...
    context 'when state is found' do
      it 'assigns the requested state and county details to @state and @county_details' do
        ...
        expect(assigns(:county_details)).to eq(@counties.index_by { |county| county.std_fips_code.rjust(3, '0') })
      end
    end
  ...
  describe '#county' do
    context 'when state and county are found' do
      before do
        ...
        @county = instance_double(County, std_fips_code: '001', state: @state)

        allow(State).to receive(:find_by).and_return(@state)
        allow(County).to receive(:find_by).and_return(@county)
        allow(@state).to receive(:counties).and_return([@county])
        allow(@county).to receive(:std_fips_code).and_return('001')
      end
      ...
      it 'assigns the requested state, county, and county details to @state, @county, and @county_details' do
        ...
        expect(assigns(:county_details)).to eq(@state.counties.index_by { |county| county.std_fips_code.rjust(3, '0') })
      end
    end
